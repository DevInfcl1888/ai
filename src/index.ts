...
import {saveDefaultVal, getDefaultVal, saveAiDataToUser, saveGlobalValue, updateAIData, getAllGlobalData, addPrice, getPrice} from "./controller/defaultVal.controller";
...
//       if (startTime && endTime) {
//         const duration = new Date(endTime).getTime() - new Date(startTime).getTime();
//         callDurationSeconds = Math.round(duration / 1000); // Convert to seconds
//         callDurationMinutes = Math.round(callDurationSeconds / 60); // Convert to minutes
//       } else if (liveTrackedDuration > 0) {
//         callDurationSeconds = liveTrackedDuration; // Use live tracked duration in seconds
//         callDurationMinutes = Math.round(callDurationSeconds / 60);
//       }
...
// // API endpoint to get live call duration
// app.get("/live-call-duration/:callId", async (req, res) => {
// const callId = req.params.callId;
...
// Additional utility function to get user call statistics
